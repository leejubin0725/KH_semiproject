...
.delete-btn::before {
   ...
   height: 12px;
   background: #FFFFFF;
   mask-image: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>');
   mask-size: cover;
}
...
<body>
   ...
   <script>
      function deleteRow(orderNo, button) {
         if (confirm("정말로 이 항목을 삭제하시겠습니까?")) {
            // 이벤트 전파 중지
            event.stopPropagation();
            
            $.ajax({
               url: `${contextPath}/order/del/\${orderNo}`,
               type: 'POST',
      ...
      window.onload = function() {
         document.querySelectorAll('tbody tr').forEach(row => {
            row.addEventListener('click', function(event) {
               // 삭제 버튼 클릭 시 이 이벤트를 실행하지 않도록 체크
               if (event.target.classList.contains('delete-btn')) {
                  return;
               }
               
               const orderNo = row.getAttribute('data-id');
               const url = `${contextPath}/order/detailProduct/\${orderNo}`;
